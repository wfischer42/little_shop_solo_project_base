...
RSpec.describe 'Merchant Orders' do
  ...
  context 'merchant user' do
    ...
    it 'displays data for an order when viewing the order show page' do
      ...
      inventory_item_1 = create(:inventory_item, merchant: merchant_1)
      inventory_item_2 = create(:inventory_item, merchant: merchant_2)
      order_1 = create(:order, user: @user)
      oi_1 = create(:order_item, order: order_1, inventory_item: inventory_item_1)
      oi_2 = create(:order_item, order: order_1, inventory_item: inventory_item_2)
      ...
      expect(page).to_not have_content(inventory_item_2.item.name)
      old_inventory = inventory_item_1.inventory
      within "#orderitem-details-#{oi_1.id}" do
        expect(page).to have_content(inventory_item_1.item.name)
      ...
      item_check = InventoryItem.find_by(item_id: inventory_item_1.item.id)
    ...
    it 'blocks me from fulfilling an order if I do not have enough inventory' do
      ...
      inventory_item_1, inventory_item_2 = create_list(:inventory_item, 2, merchant: merchant_1)
      order_1 = create(:order, user: @user)
      oi_1 = create(:order_item, quantity: 100, order: order_1, inventory_item: inventory_item_1)
      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(merchant_1)
